...
/// The metadata/mute slew has one authority in ChannelState. The binaural
/// handoff receives its exact start/rate/end segment, so a callback that
/// straddles the target boundary must hold the target instead of stretching
/// the final fraction of the fade to the callback edge.
#[test]
fn binaural_gain_slew_is_callback_partition_invariant() {
    ...
    // Both schedules cover 1200 samples. The irregular schedule deliberately
    // puts a 273-sample callback across the 960-sample slew endpoint.
