...
finish_path = root / "omniphony-renderer/realtime_ffi/src/listening_finish.rs"
tray_path = root / "omniphony-renderer/windows_installer/endpoint_apo/OmniphonyTray.ps1"

finish = r'''//! Noire X-specific post-EQ transient finishing for the Windows Current path.
//!
...
for old, new in replacements:
    count = tray.count(old)
    if count != 1:
        raise RuntimeError(f"tray anchor expected once, found {count}: {old[:60]!r}")
