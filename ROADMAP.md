...
- physically validate the authored-metric direct path now that metric XYZ bypasses scene scaling and drives ear-specific acoustic parallax plus bounded equal-power distance-dependent ILD; A/B the separately switchable finite-point-source interaural arrival-timing candidate against the direction-only baseline, and compare the authored-metric path against range-dependent HRTF/BRIR references before adding any near-field spectral model;
