...
void WINAPI OnEventRecord(PEVENT_RECORD record) {
    ...
    if (status != ERROR_INSUFFICIENT_BUFFER || infoSize == 0) {
        std::wcout << L"LICENSE_V13_TDH_INFO_ERROR\t" << status << L'\n';
        return;
    }
    ...
    std::wcout << L"LICENSE_V13_EVENT\t" << eventName << L'\n';
    ...
    for (ULONG index = 0; index < propertyCount; ++index) {
        ...
        if (sizeStatus != ERROR_SUCCESS) {
            std::wcout << L"LICENSE_V13_FIELD_ERROR\t" << propertyName << L"\tSIZE_STATUS=" << sizeStatus << L'\n';
            continue;
        }
        ...
        if (valueStatus != ERROR_SUCCESS) {
            std::wcout << L"LICENSE_V13_FIELD_ERROR\t" << propertyName << L"\tVALUE_STATUS=" << valueStatus << L'\n';
            continue;
        }

        std::wcout << L"LICENSE_V13_FIELD\t" << propertyName << L"\t"
...
int CaptureChild(const std::wstring& readyName,
    ...
    statusFile << L"LICENSE_V13_START_STATUS\t" << status << L'\n';
    if (status != ERROR_SUCCESS) {
        statusFile.flush();
        SetEvent(readyEvent);
        CloseHandle(readyEvent);
    ...
    status = EnableTraceEx2(sessionHandle, &kLicenseProvider, EVENT_CONTROL_CODE_ENABLE_PROVIDER,
                            TRACE_LEVEL_VERBOSE, ~0ULL, 0, 0, nullptr);
    statusFile << L"LICENSE_V13_ENABLE_STATUS\t" << status << L'\n';
    statusFile << L"LICENSE_V13_READY\t" << (status == ERROR_SUCCESS ? 1 : 0) << L'\n';
    statusFile.flush();
    SetEvent(readyEvent);

    if (status == ERROR_SUCCESS) {
        Sleep(kCaptureWindowMs);
    }

    const ULONG disableStatus = EnableTraceEx2(
        sessionHandle, &kLicenseProvider, EVENT_CONTROL_CODE_DISABLE_PROVIDER,
        TRACE_LEVEL_NONE, 0, 0, 0, nullptr);
    statusFile << L"LICENSE_V13_DISABLE_STATUS\t" << disableStatus << L'\n';
    statusFile << L"LICENSE_V13_STOP_CALL_BEGIN\t1\n";
    statusFile.flush();

    EVENT_TRACE_PROPERTIES stopProperties{};
    stopProperties.Wnode.BufferSize = sizeof(stopProperties);
    ...
    statusFile << L"LICENSE_V13_STOP_STATUS\t" << stopStatus << L'\n';
    statusFile << L"LICENSE_V13_CAPTURE_END\t1\n";
    statusFile.flush();
...
bool DecodeEtl(const std::wstring& etlPath) {
    ...
    if (trace == INVALID_PROCESSTRACE_HANDLE) {
        std::wcout << L"LICENSE_V13_DECODE_OPEN_ERROR\t" << GetLastError() << L'\n';
        return false;
    }

    const ULONG processStatus = ProcessTrace(&trace, 1, nullptr, nullptr);
    std::wcout << L"LICENSE_V13_DECODE_PROCESS_STATUS\t" << processStatus << L'\n';
    CloseTrace(trace);
...
int ParentProbe() {
    ...
    std::wcout << L"LICENSE_V13_PROBE_BEGIN\t1\n";
    std::wcout << L"LICENSE_V13_PROVIDER_GUID\t{030FA765-F1AE-448E-955B-44D76073E1DC}\n";

    const auto endpointId = DefaultRenderEndpointId();
    std::wcout << L"LICENSE_V13_ENDPOINT_ID\t" << endpointId << L'\n';
    if (endpointId.empty()) return 30;

    const std::wstring suffix = std::to_wstring(GetCurrentProcessId()) + L"-" + std::to_wstring(GetTickCount64());
    const std::wstring readyName = L"Local\\OmniphonyLicenseV13Ready-" + suffix;
    const std::wstring sessionName = L"OmniphonySpatialLicenseV13-" + suffix;
    ...
    const std::wstring base = std::wstring(tempPath) + L"OmniphonyLicenseV13-" + suffix;
    ...
    std::wcout << L"LICENSE_V13_ELEVATION_REQUESTED\t1\n";
    const BOOL launched = ShellExecuteExW(&shell);
    std::wcout << L"LICENSE_V13_ELEVATED_CHILD_LAUNCHED\t" << (launched ? 1 : 0) << L'\n';
    if (!launched) {
        CloseHandle(readyEvent);
    ...
    std::wcout << L"LICENSE_V13_READY_WAIT\t" << readyWait << L'\n';
    ...
    Sleep(300);
    const DWORD notifyExit = RunPackagedCommand(L"notify", L"LICENSE_V13_NOTIFY");
    const std::wstring selectArgs = L"select \"" + endpointId + L"\"";
    const DWORD selectExit = RunPackagedCommand(selectArgs, L"LICENSE_V13_SELECT");
    std::wcout << L"LICENSE_V13_NOTIFY_EXIT\t" << notifyExit << L'\n';
    std::wcout << L"LICENSE_V13_SELECT_EXIT\t" << selectExit << L'\n';
    std::wcout << L"LICENSE_V13_WAITING_FOR_CAPTURE_END\t1\n";

    const DWORD childWait = WaitForSingleObject(shell.hProcess, 15000);
    std::wcout << L"LICENSE_V13_CHILD_WAIT\t" << childWait << L'\n';
    DWORD childExit = STILL_ACTIVE;
    GetExitCodeProcess(shell.hProcess, &childExit);
    std::wcout << L"LICENSE_V13_CHILD_EXIT\t" << childExit << L'\n';
    CopyTextFileToConsole(statusPath);

    const auto fileSize = std::filesystem::exists(etlPath) ? std::filesystem::file_size(etlPath) : 0;
    std::wcout << L"LICENSE_V13_FILE_BYTES\t" << fileSize << L'\n';
    const bool decoded = childWait == WAIT_OBJECT_0 && fileSize > 0 && DecodeEtl(etlPath);
    std::wcout << L"LICENSE_V13_DECODED\t" << (decoded ? 1 : 0) << L'\n';

    CloseHandle(shell.hProcess);
    CloseHandle(readyEvent);
    DeleteFileW(etlPath.c_str());
    DeleteFileW(statusPath.c_str());
    std::wcout << L"LICENSE_V13_PROBE_END\t1\n";
...
int wmain(int argc, wchar_t** argv) {
    ...
    try {
        ...
        std::wcerr << L"LICENSE_V13_HRESULT\t0x" << std::hex << std::uppercase
                   << static_cast<std::uint32_t>(error.code().value) << std::dec
                   << L"\t" << error.message().c_str() << L'\n';
    } catch (const std::exception& error) {
        std::cerr << "LICENSE_V13_EXCEPTION\t" << error.what() << '\n';
    }
    return 99;
}
